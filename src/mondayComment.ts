...
export async function generateMondayComment(
  ...
  return `Comentário criado por: <strong>${userName}</strong> a partir de um  <a href="${pullRequest.html_url}" target="_blank" rel="noopener noreferrer">Pull Request</a>  via API\n\n${marked(mondayComment)}`;
}
